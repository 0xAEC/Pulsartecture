...
#if defined(CUDA_ARCH) && CUDA_ARCH >= 530
template <> device forceinline __half CudaMax<__half>(__half a, __half b) { return __hmax(a, b); }
#endif
//----------------------------------------------------------------------------
...
template <typename scalar_t>
device forceinline scalar_t warp_reduce_sum(scalar_t val) {
...
template <typename scalar_t>
device forceinline scalar_t warp_reduce_max(scalar_t val) {
// Requires power-of-2 warp size (standard)
#pragma unroll
for (int offset = warpSize / 2; offset > 0; offset /= 2) {
...
device forceinline scalar_t block_reduce_sum(scalar_t val, scalar_t* shared_reduce_tmp) {
...
template <typename scalar_t, int BLOCK_SIZE>
device forceinline scalar_t block_reduce_max(scalar_t val, scalar_t* shared_reduce_tmp) {
static_assert((BLOCK_SIZE % warpSize == 0), "BLOCK_SIZE must be a multiple of warpSize for this reduction.");
constexpr int num_warps = BLOCK_SIZE / warpSize;
int lane_id = threadIdx.x % warpSize;
int warp_id = threadIdx.x / warpSize;
...
//----------------------------------------------------------------------------
template <typename scalar_t>
device forceinline scalar_t vectorized_sq_distance(
const scalar_t* restrict v1,
const scalar_t* restrict v2,
int dim)
{
...
//----------------------------------------------------------------------------
template <typename scalar_t, int BLOCK_SIZE>
device void load_shared_lkeys(
const scalar_t* restrict learnable_keys_ptr, // Base pointer for the relevant key block ((Nb,Dk) or (B,Nb,Dk)[b,:,:])
...
//----------------------------------------------------------------------------
template <typename scalar_t, int BLOCK_SIZE>
global void idam_retrieve_kernel(
const scalar_t* restrict query_keys_ptr,
const scalar_t* restrict buffer_ptr,
const scalar_t* restrict learnable_keys_ptr, // Base pointer for keys
scalar_t* restrict retrieved_values_ptr,
scalar_t* restrict attn_weights_ptr,        // Can be nullptr if not needed
...
//----------------------------------------------------------------------------
template <typename scalar_t, int BLOCK_SIZE>
global void idam_update_kernel(
...
if (update_lkeys_flag && keys_are_shared) {
    ...
    // No explicit stream synchronization needed here. The PyTorch operations
